...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    params: ['recordId: string;', 'metricConfigId: string;', 'score?: object;', 'userId?: string;'],
    response:
      '{ metricConfigId: string; recordId: string; score: object; userId: string; validationErrors?: { message: string; path: string; }[]; }',
    markdown:
      "## upsert\n\n`client.scores.upsert(recordId: string, metricConfigId: string, score?: object, userId?: string): { metricConfigId: string; recordId: string; score: object; userId: string; validationErrors?: object[]; }`\n\n**put** `/records/{recordId}/scores/{metricConfigId}`\n\nCreate or update a Score for a given Record and MetricConfig. If a Score with the specified Record ID and MetricConfig ID already exists, it will be updated. Otherwise, a new Score will be created. The score provided should conform to the schema defined by the MetricConfig; otherwise, validation errors will be reported.\n\n### Parameters\n\n- `recordId: string`\n\n- `metricConfigId: string`\n\n- `score?: object`\n  The score of the Record, as arbitrary JSON. This data should ideally conform to the output schema defined by the associated MetricConfig. If it doesn't, validation errors will be captured in the `validationErrors` field. Omit `score` to leave the Score in a pending (ungraded) state — supported only for human metrics, e.g. to queue a record for manual grading.\n\n- `userId?: string`\n  Who created the Score, e.g. a user ID when submitting on behalf of a human grader. Must be a member of your organization. Only supported for human metrics and only together with `score`; defaults to `background-job`.\n\n### Returns\n\n- `{ metricConfigId: string; recordId: string; score: object; userId: string; validationErrors?: { message: string; path: string; }[]; }`\n  A Score represents the evaluation of a Record against a specific MetricConfig. The actual `score` is stored as flexible JSON. While any JSON is accepted, it is expected to conform to the output schema defined by the MetricConfig. Any discrepancies will be noted in the `validationErrors` field, but the Score will still be stored.\n\n  - `metricConfigId: string`\n  - `recordId: string`\n  - `score: object`\n  - `userId: string`\n  - `validationErrors?: { message: string; path: string; }[]`\n\n### Example\n\n```typescript\nimport Scorecard from 'scorecard-ai';\n\nconst client = new Scorecard();\n\nconst score = await client.scores.upsert('a1b2c3d4-e5f6-7890-1234-567890abcdef', { recordId: '777' });\n\nconsole.log(score);\n```",
    perLanguage: {
      typescript: {
