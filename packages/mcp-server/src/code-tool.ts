...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { SCORECARD_API_KEY: requireValue(readEnv('SCORECARD_API_KEY') ?? client.apiKey, 'set SCORECARD_API_KEY environment variable or provide apiKey client option'), SCORECARD_BASE_URL: readEnv('SCORECARD_BASE_URL') ?? readEnv('SCORECARD_ENVIRONMENT') ? undefined : client.baseURL ?? undefined };
