...
async function fetchLatestInstructions(stainlessApiKey: string | undefined): Promise<string> {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/scorecard',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the scorecard MCP server. You will use Code Mode to help the user perform
