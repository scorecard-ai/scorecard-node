...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['SCORECARD_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: Scorecard) => {
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new Scorecard({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'SCORECARD_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['SCORECARD_BASE_URL'] = undefined;
    });
